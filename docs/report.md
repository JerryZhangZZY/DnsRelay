...
On finishing decapsulating the request, the resolver first check the question type. 
...
For flow control consideration, one address of the queried domain is randomly selected.
...
There are some other types of DNS question in the socket, like CNAME, MX, or inverse DNS. These types only requires simple relay because they do not need flow control or blacklisting, and are not included in cache. The program simply forward relay response back to the client, and no need to extract the addresses and reformat the message.
